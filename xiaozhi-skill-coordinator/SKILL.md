...
description: >
  让错题本、费曼测试、康奈尔笔记与学习计划、时间专注数据在明确任务下协同工作。
...
三SKILL联动协调器
    ...
    ←── 时间与专注力教练（专注与时间使用数据）
    ──→ IM智能提醒（已授权的复测、任务、探索、确认提醒）
    ──→ 学习DNA档案（仅在用户同意时写入月报摘要或成长轨迹摘要）
    ──→ 每周学习复盘（仅传递全景周数据摘要）
```

协作边界：
- 不默认读取全部已安装SKILL的数据，只读取当前任务需要的SKILL摘要
- 不默认把联动结果写回学习DNA或提醒系统，涉及长期保存或定时提醒时需单独授权
- 不向未声明用途的第三方SKILL传递汇总数据
- 用户要求关闭某个联动源时，必须尊重并改用当前会话信息

最小字段示例：
- 错题本：知识点、错误类型、攻克状态
- 费曼测试：掌握状态、待验证项
- 康奈尔笔记：是否存在相关笔记、简要线索问题
- 学习计划：任务完成率、延期情况
- 时间与专注力：时段偏好、执行阻力摘要
