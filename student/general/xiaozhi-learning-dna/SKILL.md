...
description: >
  ...
  本 SKILL 是档案的存储与授权层，自己只产出学习情绪维度（§七，需 emotionTrackingConsent）与成长里程碑（只由已写入的证据或学生自述触发，需确认）。错因、理解深度由错题本与费曼学习法经交接写入；不做错题分析、不做理解验证、不发提醒；普通答疑不读档案。
...
| 动作 | 读什么（最小必要） | 写什么 | 谁确认 |
|---|---|---|---|
| 普通答疑中参考档案 | 与当前题目直接相关的 1-3 个弱项字段 | 不写 | `profileEnabled=true` + 本次未说"不要读取"；学段需监护人时监护人同意在先 |
