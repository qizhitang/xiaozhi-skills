...
description: >
  ...
  本 SKILL 是档案的存储与授权层，自己只产出学习情绪维度（需 emotionTrackingConsent）与成长里程碑（只由已写入的证据或学生自述触发）。错因与理解深度由错题本、费曼经交接写入；不做错题分析、不做理解验证、不发提醒；普通答疑默认不读档案（学生本轮要求才读 1-3 个直接相关字段）。
  所有开关默认关闭；未获同意只用当前会话信息；约 14 周岁以下需监护人同意；说话人未确认时受限：不读不写不改授权。
