...
> 适用学段：小学中段（仅前两跳）、小学高段（前三跳）、初中与高中（全流程）。
> 本文档定义 `xiaozhi-feynman-learning` 核心工作流（四跳验证 + 第五跳批判性验证 + 苏格拉底挑战者模式）的完整状态转移逻辑。
> 任何一跳出现挫败信号时，先做危机信号检查（`shared/crisis-exception.md`），再决定是否进入熔断。
...
stateDiagram-v2
    ...
    S_EVALUATE --> [*] : 输出评估等级（写回不在流程内：学生说要、且 crossSkillSharing 为 true，才发交接）
```
...
| 项 | 说明 |
|---|---|
| **进入条件** | 学生说"我来给你讲讲"/"我懂了想测试一下"/"帮我检验一下"；错题本攻克后、学生同意“测一下”；考前自检 |
...
|---|---|
| **进入条件** | 前四跳基本通过 |
| **学段适配** | **小学段**：找茬游戏化（"有个同学说[故意说错的结论]，你觉得哪里不对？"），不强制通过；**初中及以上**：边界与逻辑验证（"这套说法有没有适用边界？"） |
| **自循环条件** | 学生无法找到漏洞/边界 → 进一步提示 |
| **退出条件** | 初中及以上：学生能识别边界条件或找到逻辑漏洞；小学段：不强制通过，尝试后即进入评估 |
| **跳过条件** | 小学段若学生明显无法理解"找茬"，可跳过第五跳直接进入评估，**评级不封顶**——前四跳（小学高段为前三跳）都通过即可给 A |
...
|---|---|
| **进入条件** | 学生在任何跳离线或中断 |
| **AI动作** | 在本次会话内保留当前跳数与已通过项（会话结束即丢弃，不做跨会话续接） |
...
| **AI动作** | 给出评估等级（A/B/C/D）+ 建议后续动作。**写回不是本状态的动作**：只有学生本轮明确说“记下来”且 `crossSkillSharing` 为 true，才发 `subject_profile_writeback`，且只带等级与卡点类型，不带学生原话 |
| **评估标准** | 初中及以上：A=四跳全通过 + 第五跳通过；B=四跳通过但第五跳未通过；C=1-2 跳通过；D=第1跳就卡住。<br>小学高段：A=前三跳通过（第五跳为加分尝试，跳过不降级）。<br>小学中段：A=前两跳通过。<br>写回档案的是 `depth`（会复述/会解释/真正掌握，`shared/vocab.md §6`），等级只是过程标签 |
| **后续** | 流程结束 |
...
> 下面这份 JSON 只用于本次会话内在跳与跳之间续接，会话结束即丢弃。
> `studentAnswer` 的原话不进任何档案；进档案的只有“掌握度等级 + 卡点类型”，且要走上一节的同意门。
...
| 学生在第4跳后说"太累了不想继续" | S4 | 先做危机信号检查；属于普通疲倦则 S_PAUSED 或 S_EVALUATE（按学生意愿，已完成 4 跳按本学段标准评级） |
