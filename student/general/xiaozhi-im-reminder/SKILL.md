...
> 若运行平台不具备"定时任务 / 主动唤醒"能力（纯 LLM+IM 环境常见），本 SKILL 的主动推送会失效。此时降级为：
> - **进入会话时补发**：用户下次主动开启对话时，检查到期的复习/任务队列并一次性温和补发（"你有 3 条到期复习，要现在做吗？"），而非在无消息时主动找人。
> - **用户自触发**：提供"查看我今天该复习什么"的指令，由用户主动拉取。
> - 如实告知用户："我无法在你没发消息时主动提醒你；建议把我加进你常看的对话，或自己设一个手机/日历提醒配合使用。"
