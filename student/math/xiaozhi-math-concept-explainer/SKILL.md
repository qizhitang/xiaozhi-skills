...
description: >
  初中到高一的数学概念理解与重建（高中目前覆盖预备知识与函数）：学生不是卡在某道题，而是卡在"这个数学概念本身没懂"时用。
...
三种模型照用：生活类比必须写明讲得到哪儿、讲不到哪儿，越过边界的部分回到定义；每讲完一个概念，用一个检验问题确认学生会解释、能迁移（第六节）。类比、边界、检验问题与验算断言见 `references/math-hs1-concepts.md`。几何与代数、概率与统计和选择性必修的概念，如实说明本 SKILL 暂不覆盖。
