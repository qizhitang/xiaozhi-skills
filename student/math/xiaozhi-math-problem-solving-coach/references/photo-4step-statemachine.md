...
> 覆盖"拍题→意图识别→追问并出同类题→归档"全流程及中断恢复。
>
> **两条硬规则：**
...
stateDiagram-v2
    ...
    S3_CHECK --> S6_ARCHIVE : 做对，归档
    S3_CHECK --> S3_PROBE : 连续 2 道同类题做错 → 回到根因定位（可能选错了分支）

    S6_ARCHIVE --> [*] : 询问是否记入错题本，同意则生成交接条目
    ...
    S_PAUSED --> S3_PROBE
    S_PAUSED --> S3_SIMILAR
    S_PAUSED --> S6_ARCHIVE : 两轮无回复 → 收尾归档
...
**全局退出规则：**
- **两轮无回复即收尾**：输出一句话小结（这道题卡在哪、下一步做什么），进 S6_ARCHIVE，不再追问、不追加提醒。
...
| 项 | 说明 |
|---|---|
| **进入条件** | 学生发来题目（图片/文字）/"我做错了这道题"/"我不知道怎么做"/"帮我看看这道题" |
...
| **退出条件** | 学生给出文字条件 → S1_PHOTO；两轮无回复 → 收尾归档 |
...
| **断点恢复** | 记录"当前轮次 + 已获得的回答 + 待追问方向"，恢复时说"上次我们分析到[X]，接着来" |
...
|---|---|
| **进入条件** | 学生做了同类题 |
| **退出条件** | 做对 → S6_ARCHIVE；做错 → S3_SIMILAR 再出 1 道（难度不升）；连续 2 道错 → 回 S3_PROBE 重新定位根因（很可能 S2 选错了分支） |
...
| 学生两轮没有回复 | 任意 | S6_ARCHIVE：一句话小结 + 归档，不追加提醒 |
