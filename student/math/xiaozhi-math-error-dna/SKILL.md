...
description: >
  初中到高一的数学错题根因深度分析（高中部分目前覆盖高一衔接段：预备知识与函数）：把错题本判定的通用四维，细化为数学子类型（B/C/R/M + 两位编码）并做跨维度关联。
...
依据《普通高中数学课程标准日常修订版（2017 年版 2025 年修订）》必修课程的主题一"预备知识"与主题二"函数"，即高一上学期的主要内容。学生说明在读高一，或初中毕业后做衔接预习时用本节；初中学生仍用维度表第二至五节的子类型。

| 学段 | 范围 |
|---|---|
| 初中 | 本 SKILL 的主体：维度表第二至五节的子类型 |
| 高中 | 目前覆盖高一衔接段：集合、常用逻辑用语、不等式、从函数观点看一元二次方程与不等式；函数概念与性质、幂函数、指数函数、对数函数、三角函数、函数应用。用维度表第九节的子类型（B10—B18、C09—C11、M07—M09、R07—R08）；几何与代数、概率与统计和选择性必修的内容如实说明暂不覆盖，只做通用四维分类 |
...
突破流程与初中相同（第四节）：先用纯净版题定位；B 类转数学概念解释器，M07、M08 转数学解题教练，M09 转应用题数学建模教练；间隔 3 天以上两次独立做对才算攻克。纯净版题与变式题生成前按 shared/ai-item-check.md 自检。

---

## 十、参考资源

- `references/math-error-dimension-table.md` — 数学错因维度表（四维×子类型分类体系，含跨维度关联规则与追踪标准；第九节为高一衔接段的子类型，含 CI 验算断言）
