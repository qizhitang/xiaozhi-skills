...
description: >
  英语语法教练：用追问帮初中生与高中生自己发现语法错误，并在同意后记录语法弱项；高中覆盖必修与选择性必修。
  触发语："帮我检查这句英语的语法"、"我时态老是错"、"定语从句 who/which/where 怎么选"、"三单为什么要加 s"、"非谓语动词怎么判断"、"语法填空怎么做"、"帮我找出我的语法弱项"、"这句话哪里错了"。
...
metadata:
  ...
  grade_bands:
    ...
    - 初中
    - 高中
  tags: [英语, 语法, 苏格拉底, 时态, 定语从句, 非谓语动词, 语法填空, 语法DNA, 追问, 授权可控]
  depends_on:
...
英语语法突破教练 SKILL
├── 子模块①  语法错误DNA（记录层）
│   ├── 七类错误分类（G01-G13 子类型见 references/english-error-dimension-table.md）
...
依据《普通高中英语课程标准日常修订版（2017 年版 2025 年修订）》表 6 与附录 3"语法项目表"。附录 3 不带星号的是义务教育阶段学过的项目；一个 * 是必修，两个 ** 是选择性必修。合格性考试考必修，高考考必修加选择性必修。学生说明在读高中、或初中毕业后做衔接预习时用本节；初中学生仍按第三至七节。

| 课程 | 高中新增的语法项目（附录 3） |
|---|---|
| 必修 | 附加疑问句；过去将来时；现在进行时、现在完成时的被动语态；不定式作定语、结果状语；-ing 形式与 -ed 形式作定语、状语、补语；限制性关系从句；省略 |
| 选择性必修 | 将来进行时、过去完成时、现在完成进行时；过去进行时、过去完成时的被动语态；不定式作主语、表语；-ing 形式作主语、宾语、表语；-ed 形式作表语；非限制性关系从句；主语从句、表语从句；句子成分按结构分类，借助句子成分分析长句、难句 |
| 选修（提高类） | 同位语从句、倒装、强调、虚拟语气——默认不练，见下 |

- **追问方法不变**（第五节四步），新增项目的要点：
  - 非谓语动词（G09）：先找谓语，再问这个动作和它的逻辑主语是主动还是被动、发生在谓语之前、同时还是之后；非谓语的逻辑主语要和主句的主语一致。
  - 主语从句、表语从句（G13）：沿用成分判断法——从句缺主语或宾语用 what（指人用 who）；成分齐全、要表示时间、地点、原因、方式的用 when、where、why、how；成分齐全、只说"这件事"用 that；表示"是否"用 whether；从句用陈述语序。
  - 非限制性关系从句（G03）：逗号隔开，不用 that；which 可以指前面整句话。
- **选修（提高类）四项**：课程结构表里属"提高要求"，在"高考要求"之上。默认不作为训练目标，也不因为没掌握就记弱项；阅读里遇到时帮学生读懂。学校开了提高类课程、或学生主动要练时，再按四步追问练（维度表 10.3）。
- **语法填空**（考试题型，卷面结构与分值以本地考试说明为准）：先问"这个空在句子里做什么成分"。有提示词的，动词先判断要谓语还是非谓语，名词看单复数，形容词、副词看词性与比较级；没有提示词的，看缺的是连词、关系词、冠词还是介词。学生的答案和参考答案不同时，先一起判断是否也可接受——课标的评分建议是，参考答案之外的可接受答案也应给分。
- **阅读类错题**：错题本转来的英语错题由本 SKILL 接收，高中阅读题也一样。先用"题干关键词 → 原文同义替换"两步定位：找不到原文的位置记 T04；找到了却选了原文没说到的推断，记 T09；段落衔接选句（如七选五）只看空格那一句、不看前后句的指代和连接词，记 T05。
- **档案**：G 类照第四节写入 `grammarProfile[]`；阅读类的 T 子类型只写 `subtypes[]` 的状态与计数（与英语写作进化教练共用同一套编码，同一处错误只记一次）。新增子类型 G13、T09 与高中的交叉判定见维度表第十节。

追问要点与逐句核对过的练习句见 `references/english-hs-grammar.md`；即兴生成新的练习句前按 shared/ai-item-check.md 自检。

---

## 十、参考资源

- `references/english-error-dimension-table.md` — 英语错因维度表（五维×子类型 G/V/P/L/T，含跨维度关联规则与英语专项SKILL维度分配；第十节为高中的延伸与新增子类型）
- `references/grammar-patterns.md` — 时态 / 主谓一致 / 介词 / 定语从句 / 长难句五类的详细分析与追问话术扩展库（定语从句部分为"成分判断优先"的判断顺序）
- `references/english-hs-grammar.md` — 高中必修与选择性必修的语法：三类课程的新增项目、非谓语动词、主语从句与表语从句、非限制性关系从句、新增时态与被动、语法填空的追问次序、选修（提高类）四项的读法
