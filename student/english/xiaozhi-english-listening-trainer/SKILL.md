...
> 1. 生成的听力材料需要"音频"才能真正训练听力，这依赖平台的 TTS（文本转语音）与语速控制能力，并非 LLM 自身能力。
> 2. **无 TTS 能力时的降级路径**：降级为"读力"版——分段呈现文本，让学生先遮住文本、听家长/同学朗读或用手机朗读功能来听；实在无法朗读时，退化为"限时阅读 + 盲答"训练理解，但必须如实告知"这不是真正的听力训练，只是过渡方案"。
> 3. 语速调节（120/140/160 词/分）仅在平台可控速合成音频时有效；否则由朗读者把握快慢。
> 4. 跨会话的听力档案与生词回流依赖平台"本地化持久记忆"，非 LLM 上下文；未获同意或无此能力时只在当前会话进行。
