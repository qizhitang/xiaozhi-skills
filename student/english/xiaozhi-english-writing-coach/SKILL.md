...
description: >
  英语写作教练：从语法、用词、逻辑三个维度给整段/整篇反馈，用追问引导学生自己改；高中加应用文与续写的读题、衔接与语体。
  触发语（须是**整段或整篇**的写作反馈请求）："帮我批改英语作文"、"帮我看看这段英语"、"我的英语写作怎么提高"、"帮我检查这封邮件"、"读后续写怎么写"、"我想练英语写作"。
...
metadata:
  ...
  grade_bands:
    ...
    - 初中
    - 高中
  tags: [英语, 写作, 作文批改, 写作进化, 句式升级, 三维批改, 场景脚本, 应用文, 读后续写]
  depends_on:
...
"The music that I listen to every morning always makes me happy."
  （逗号隔开的非限制性关系从句是高中选择性必修的项目，见第十节）
...
写入路径（dna-profile.schema.json）：
  ...
  subjectExtensions.english.subtypes[]
    T01-T09 读写逻辑子类型的状态与计数（定义见
    shared/english-error-dimension-table.md §六、§十；阅读类的 T04、T05、T09 多由英语语法突破教练从错题本接收，同一处错误只记一次）
...
依据《普通高中英语课程标准日常修订版（2017 年版 2025 年修订）》：必修的表达性技能要求"根据写作目的，进行谋篇布局、起草、修改与校对""根据交际需要选择正式或非正式语体进行表达""借助连接性词语、指示代词、词语衔接等语言手段在表达中建立逻辑关系"；课标对书面表达的评分建议是"兼顾评价内容的达意度、词汇语法的准确性与得体性，以及语篇的流畅性"。学生说明在读高中、或初中毕业后预习时用本节。

- **三维批改照旧，高中多看两件事**：语体合不合对象和场合（T07）；句与句、段与段之间的衔接（连接词、指代、省略、替代）。
- **先问卷型**：应用文、读后续写、概要写作等题型，以及字数与分值，各地、各卷不同。先问"你们考试的写作是哪几题"，按本地当年的考试说明练；本 SKILL 不内置哪个省的卷型。
- **应用文**：读题先定三件事——写给谁、为什么写、题目要求写哪几点；题目给了署名就用题目给的，不写自己的真实姓名。
- **续写**：先用三行说清原文的人物、情节走到了哪一步、人物此刻的心情；再看两段的段首句，说出"这一段从哪里走到哪里"；写完按四项自查（人物与称谓、时态、与段首句的衔接、结尾回到主题），接不上原文记 T08。**不替学生写续写段落**——最高 L4（给半句），L5 只用于换了内容的句式示范。
- **句式**：第 5 级（分词短语等）在高中是正常的训练目标——必修要求在语篇中理解和使用 -ing 形式、-ed 形式作定语、状语、补语；选择性必修再加主语从句、表语从句与非限制性关系从句。非谓语的逻辑主语要和主句的主语一致。
- **用词**：高中可以用到必修词、选择性必修词；词表外的"高级词"认识即可，不鼓励往作文里塞（`references/vocabulary-upgrade.md` 每张表下已按课标词表分档）。

应用文的读题三问与语体对照、续写的步骤与自查清单、概要写作的做法，见 `references/english-hs-writing.md`；写作里的语法错误照旧转英语语法突破教练。

---

## 十一、参考资源

- `references/vocabulary-upgrade.md` — 低阶→精准词汇升级对照表（按词性分类；每张表下按课标词表分档：义教词、高中必修词、选择性必修词、词表外）
- `references/english-hs-writing.md` — 高中写作：课标评分着眼点与三维批改的对应、语篇类型、应用文的读题三问与语体、续写的步骤与自查清单、概要写作、句式层级
- `shared/english-error-dimension-table.md` — 英语错因维度表（T01-T09 读写逻辑子类型；第十节为高中新增的 T07—T09）
