...
description: >
  24小时随时待命的英语口语教练——在你允许连续跟踪时，记住发音弱点，越说越认识你。
...
> **关于“口语DNA”机制：** 本模块记录发音弱点并跨次纠错的长期记忆能力，**并不依赖**大模型自身的记忆能力，而是依托于 OpenClaw 平台底层的「本地化持久记忆机制」。

### 关于发音评估与卡壳检测的能力边界与降级

> - **音素级纠音**（重音、长短元音、连读缺失）依赖具备**语音评估能力**的多模态通道；仅有普通 ASR 语音转写时，转写会把发音"归一化"掉，看不到音素错误。**此时降级**：纠音闭环退化为用词、语法、表达、流利度层面的复盘（复用晨间热身状态机 S1 的文字降级分支），并如实告知学生"我暂时听不出你具体的发音问题，建议对照音标或找老师当面听一次"。
> - **"卡壳超过 5 秒给提示"**依赖平台实时静默检测；无此能力时改为回合制：学生说完一段再给反馈，或由学生主动说"我卡住了"来求助，而非依赖自动计时。
