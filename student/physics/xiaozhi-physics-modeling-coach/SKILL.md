...
description: >
  ...
  （识别现象→选择模型→数学表达）。核心模型覆盖初中物理主要建模场景
  （其中匀变速运动、动量守恒等属初高中衔接内容，已在正文标注"高中拓展"），
...
> ⚠️ 学段提示：加速度、v=v₀+at、s=v₀t+½at²、v²=v₀²+2as 等匀变速定量关系属**高中必修一**内容，初中不作定量要求。面向初中生时，本模型只用于建立"匀速 vs 变速"的定性图景；定量建模仅供高中或学有余力者使用。
...
> ⚠️ 学段提示：动量、动量守恒定律及其方程属**高中**内容，初中不作要求。面向初中生时只做下方"初中简化版"（碰撞前后状态分析），定量动量方程仅供高中或学有余力者。
...
核心关系：
  能量守恒：E初 = E末 + 损失（能量守恒思想初中即建立，是本模型初中重点）
  机械能守恒（无摩擦时）：E动能 + E势能 = 常数（⚠高中：机械能守恒的定量方程属高中；初中只作"动能与势能相互转化"的定性理解）
  电能守恒：W电 = W热 + W光 + W机械
