...
> 适用学段：初中（7-9 年级）与高一必修第一册（高中衔接段）。状态流转两个学段相同；高一在每一步要加的动作见 SKILL.md 第七节，高一样板题见 `physics-hs1-worked-examples.md`。
> 本文档定义 `xiaozhi-physics-problem-coach` 核心工作流（模块A：四步物理解题法）的完整状态转移逻辑。
> 覆盖"读题画图→物理建模→列式计算→检验反思→苏格拉底追问"全流程及中断恢复；
> 归档（S6）是**可选加项**，不是流程的必经终点。

> **数据边界（本状态机的前提，来自 SKILL.md §〇）**
>
> - **默认只在当前会话工作**：S0-S5 全程不需要长期档案，不写任何持久数据。
> - **OCR / 多模态识别**：只用于读出这道题的文字与图，识别结果只在本次会话里用；
>   不保存题目图片、不保存原图、不把图片内容送去其他 SKILL。
>   进入 S0 时如实说一句："我读一下你发的这张题目图，只用来看清题目。"
> - **读档案**：只在 `profileEnabled=true` 且学生这次同意时读，且只读
>   `subjectExtensions.physics.subtypes[]` 与 `subjectMap.weakKnowledgePoints[]` 里的物理条目。
>   情绪、兴趣、安全记录、其他学科分支一律不读。
> - **写**：本 SKILL 自己不写档案。要落库只有一条路——学生在 S6_ASK_ARCHIVE 明确说"记"，
>   然后发 `wrong_answer_handover` 给通用错题本。沉默不等于同意。
> - **提醒**：`reminderConsent=true` 且学生这次说"要"，才发 `reminder_enqueue`；本 SKILL 不自己发提醒。
> - 学生随时可说「查看我的解题记录」「删除我的解题记录」「这次不要记忆」。
...
stateDiagram-v2
    ...
    S5_TRANSFER --> S6_ASK_ARCHIVE : 迁移验证通过，问一句要不要归档
    S6_ASK_ARCHIVE --> S6_ARCHIVE : 学生明确说"记"
    S6_ASK_ARCHIVE --> S_END : 学生说"不用"/没回答 → 什么都不写
    S6_ARCHIVE --> S_END
    S_END --> [*] : 本次会话结束，未同意的内容不留存

    S5_PHENOMENON --> S1_DIAGRAM : 现象描述不清→回退画图
    S5_PRINCIPLE --> S2_MODEL : 原理理解有误→回退建模
    S5_TRANSFER --> S3_CALCULATE : 迁移失败→回退列式

    S4_VERIFY --> S6_ASK_ARCHIVE : 验证发现错误→先问要不要归档
    S5_SOCRATES --> S6_ASK_ARCHIVE : 追问中发现深层错误→先问要不要归档
...
| 项 | 说明 |
|---|---|
| **进入条件** | 学生**带着一道具体的题**进来：发来题目（图片/文字），或说"这道力学题不会""帮我看看这个电路题""这道题我做错了" |
| **不进入** | 泛泛说"物理好难""不想学物理"（按普通对话回应）；只问概念不带题（转物理概念直觉器）；问实验设计/方法/数据处理/误差（转物理实验思维教练）；只剩纯代数运算（转数学解题教练） |
| **AI动作** | ①识别题目内容（图片→OCR/多模态识别，识别前说一句"我读一下这张题目图，只用来看清题目"；识别结果只在本次会话用，不保存图片）②判断所属物理领域（力学/电学/光学/热学）③扫描关键词判断图景类型 |
...
| **中断处理** | 只在**同一会话内**保留已识别的题目片段，恢复时问"刚才那道[领域]题，你想继续分析吗？"；会话结束即丢弃，不跨会话记住学生做过哪道题 |
...
| **常见错误追问** | "你把电压表也画进去了——电压表电阻很大，当断路处理" / "开关S1闭合S2断开时，电路结构变了，你重新画一张等效电路" |
...
| **AI动作** | 逐一检查每个量的单位：km/h→m/s了吗？cm²→m²了吗？g→kg了吗？kW·h→J了吗？ |
| **学生预期动作** | 自检并修正单位 |
| **退出条件** | 所有单位已统一为国际单位制 → S4_VERIFY |
| **自检清单** | 长度：mm/cm→m；面积：cm²→m²（除以10000，不是100）；体积：cm³→m³；质量：g→kg；速度：km/h→m/s（除以3.6）；时间：min→s；压强：mmHg→Pa；功/能：kW·h→J（×3.6×10⁶） |
...
| **学生预期动作** | 对计算结果进行三重验证 |
| **退出条件** | 三重验证全部通过 → S5_SOCRATES；任一验证失败 → 在会话内标记错误类型 → S6_ASK_ARCHIVE（先问，学生同意才归档） |
...
| **AI话术** | "检验一下单位——等式左边要的是[压强]，单位应该是 Pa；右边 N ÷ m² 算出来是 Pa 吗？" |
| **常见问题** | 力用 N；压强 Pa = N/m²；功 J = N·m；功率 W = J/s；电功 J = V·A·s。求力却算出 J → 公式用错了 |
| **通过条件** | 等式两边量纲一致 |
...
| **常见数量级参考** | 人步行速度约1-2m/s；中学生体重约400-600N；家用电灯功率约20-100W；汽车速度约20-30m/s；一节干电池电压1.5V；家庭电路电压220V；标准大气压约 10⁵ Pa |
| **通过条件** | 结果数量级与物理常识吻合 |
...
| **常见矛盾检查** | 机械效率大于 1 → 违反能量守恒；并联电路中某条支路电流大于干路电流 → 不可能，干路电流等于各支路之和；串联电路中各处电流算出来不相等 → 串联无支路，I 处处相等；成像位置在凸透镜同侧却说是实像 → 实像必在异侧 |
| **通过条件** | 结果物理意义合理，无矛盾 |
...
| **退出条件** | 三层追问全部通过 → S6_ASK_ARCHIVE（问一句要不要把"已掌握"记下来，同意才归档）；某层追问失败 → 回退到对应步骤 |
...
### S6_ASK_ARCHIVE — 问一句要不要归档（**可选状态，不是流程的必经步骤**）

> 正常流程在 S5 追问通过后就**结束**了。归档、提醒都是可选加项，
> 学生这一次明确说要，才走 S6；不说、没回答、说"不用" → 直接 S_END，什么都不写。

| 项 | 说明 |
|---|---|
| **进入条件** | 三重验证失败 / 追问中发现深层错误 / 迁移验证通过，**且**本次会话里还没问过归档 |
| **AI动作** | 只做一件事：问一句。"这道题的错因是[一句话]，要不要交给错题本记一笔？不记也行。" |
| **退出条件** | 学生说"记" → S6_ARCHIVE；学生说"不用"/没回答/岔开话题 → S_END（不写任何东西） |
| **不做的事** | 不预先生成交接、不先写后问、不把"讲完了"当成默认同意 |

### S6_ARCHIVE — 归档（**仅在 S6_ASK_ARCHIVE 得到明确同意后进入**）

| 项 | 说明 |
|---|---|
| **进入条件** | 学生在 S6_ASK_ARCHIVE 明确说"记" |
| **AI动作** | ①把要记的条目复述一遍给学生看 ②以 `wrong_answer_handover` 交接给通用错题本（唯一收录与计数入口；**本 SKILL 自己不写档案、不数次数**） ③学生同意且需要五维深度归因时，再转物理错误DNA ④**另外**问一句要不要提醒——`reminderConsent=true` **且**学生这次说"要"，才生成一条 `reminder_enqueue`，由 IM 提醒统一发送；本 SKILL 不承诺"我会在 X 时提醒你" |
| **归档内容（上限）** | `surfaceInfo`（题目摘要、学生答案、正确答案、表面根因）、`basicDimension`（通用四维）、`physicsBasicDimension`（五维）、`subtypeId`（如 P02）、跨维度关联（如有）、建议训练方向。**不带**情绪判断、不带兴趣信号、不带其他学科内容、不带学生原话中与题目无关的部分 |
| **退出条件** | 交接发出 → 流程结束 |
...
> 下面这个对象只在本次解题过程里存在，用于中断后接着讲；会话结束即丢弃。
> 它**不是**写入 `dna-profile` 的内容——要进长期档案只有 S6_ARCHIVE 那一条路，
> 且必须先在 S6_ASK_ARCHIVE 拿到学生明确同意。
> 对象里也不放学生原话中与题目无关的内容、不放情绪判断、不放题目图片。
...
| 物理意义检查失败（结果违反基本定律） | S4_MEANING | 会话内标记为P类或C类错误 → S6_ASK_ARCHIVE（问过才归档） |
...
| S6_ASK_ARCHIVE + S6_ARCHIVE | 1-2分钟 | 学生不回应即视为不归档，直接结束（沉默不等于同意） |
...
根据学生描述的问题类型，选择对应CLAW模板：
  ...
  学生说"这个实验题我不会分析" → 模板④**实验类请求 = 转介模板**
      （实验设计/方法/数据处理/误差分析一律转出，不在本 SKILL 解答；
       只有“实验作背景的计算题”留下按①②③走）
  学生说"明天考试帮我复习电学" → 模板⑤考前突击类
      （**仅限围绕当前这道题/这个知识点的快速梳理**；
       要整章/整科复习计划的转 `xiaozhi-learning-plan`，本 SKILL 不做跨题目的复习编排）
...
归档的**唯一出口**是：`wrong_answer_handover` → 通用错题本（唯一计数权威）。
本 SKILL 不直接写物理错误DNA、不直接写学习DNA、不同时推多个目的地。
需要五维深度归因或能力维度更新的，由通用错题本按各自的授权位再向下游分发——
那是它们的同意模型，不是本 SKILL 的。
```
