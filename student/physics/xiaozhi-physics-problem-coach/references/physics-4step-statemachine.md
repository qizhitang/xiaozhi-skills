...
| **AI动作** | ①生成待确认的错题条目（验证失败时），学生点头后 ②以 `wrong_answer_handover` 交接给通用错题本（唯一收录与计数入口） ③需要五维深度归因时再转物理错误DNA ④学生的 `reminderConsent` 为 true 且本次同意时，生成一条 `reminder_enqueue`，由 IM 提醒统一发送 |
| **归档内容** | `surfaceInfo`（题目摘要、学生答案、正确答案、表面根因）、`basicDimension`（通用四维）、`physicsBasicDimension`（五维）、`subtypeId`（如 P02）、跨维度关联（如有）、建议训练方向 |
| **退出条件** | 归档完成 → 流程结束 |
