...
> 作者：小智伴学 ｜ 适用平台：WorkBuddy / SkillHub · OpenClaw / ClawHub ｜ 当前版本：**v2.5.0**
> 全库 **83 个 SKILL**（学生端 49 + 老师端 34）+ 1 个开发者工具，**221 份 references**、**6 份共享约定**、**4 份 JSON Schema**（含 8 份示例）、**3 个校验脚本**。
...
│   ├── general/     通用学习 13 个
│   ├── chinese/     语文 5 个
│   ├── math/        数学 5 个（初中 + 高一衔接段：预备知识与函数）
│   ├── english/     英语 5 个（初中 + 高中必修与选择性必修）
...
│   ├── chinese/     语文教学 3 个
│   ├── math/        数学教学 3 个（初中 + 高一衔接段）
│   ├── english/     英语教学 3 个（初中 + 高中）
│   ├── physics/     物理教学 3 个（初中 + 高一必修 1）
│   ├── chemistry/   化学教学 3 个
│   ├── history/     历史教学 2 个
...
学科 SKILL 默认基线为**初中**；小学高段可用的已在 `metadata.grade_bands` 中标明，超出学段的内容在正文中带 `⚠高中` 标注。数学、物理、化学、英语的学生端声明了高中学段：高中章节按高中课标写，不逐行标注，目前数学覆盖高一衔接段（预备知识与函数），物理覆盖高一必修第一册，化学覆盖高中必修起点（物质的量、氧化还原、离子反应），英语覆盖必修与选择性必修（选修提高类的倒装、强调等只求读懂）。化学开课的年级各地不同，技能先问学校教没教过，不按年级号一刀切（`shared/grade-bands.md` §四）。历史三个学生端技能同样覆盖初中与高中，以方法为主（史料实证、时空观念、历史解释），史事与评价口径以课程标准和统编教材为准（`SECURITY_BASELINE.md` 4.4）。生物、地理的学生端同样覆盖初中与高中：初中兼顾学业水平考试（会考）的复习，高中按选科定合格考或等级考的深度；生物涉及人体健康的内容不做诊断，地理用教材地图或标准地图，不绘制国界与行政区划（`SECURITY_BASELINE.md` 4.4）。
...
| 学科教学（20） | 语文 3（写作/阅读/文言文）｜ 数学 3（教案/错因分析/测评，含高一衔接段）｜ 英语 3（口语/听力/综合测评，含高中）｜ 物理 3（教案/实验/解题，含高一必修 1）｜ 化学 3（教案/实验/化学用语过关）｜ 历史 2（教案/命题与答题规范）｜ 生物 2（教案/生地学业水平考试复习规划）｜ 地理 1（教案） |
...
- 🧭 [学科扩展路线图](docs/roadmap-new-subjects.md) — 按各地中考与高考学科要求重排：物理高中化、化学、历史、高中生物与地理、数学高中化、英语高中化（第零批随 v2.1.13、第一批随 v2.2.0、第二批随 v2.3.0、第三批随 v2.4.0、第四批随 v2.5.0 发布；第五批已完成、待发版）
