...
```json
{
  ...
  "recipient": "xiaozhi-math-error-gene-archive",
  "payload": {
    "wrongAnswerData": {
      "errorId": "err-{{id}}",
      "subject": "math",
...
```json
{
  ...
  "handoverType": "wrong_answer_handover",
  "sender": "xiaozhi-wrong-answer-notebook",
  "recipient": "xiaozhi-physics-error-archive",
  "payload": {
    "wrongAnswerData": {
      "errorId": "err-{{id}}",
      "subject": "physics",
...
```
*注：对于高度顽固物理错题触发，`handoverTrigger` 设为 `stubborn_weakness` 并附带 `historyRefs`（如 `["err-1", "err-2"]`）；若检测到学生流露物理焦虑，`handoverTrigger` 设为 `anxiety_trigger` 并附带 `anxietySignals`；judgmentClue 在 "new_error" 类型时必填，帮助物理基因档案快速定位子类型。*
