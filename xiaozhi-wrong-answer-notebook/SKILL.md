...
depends_on: xiaozhi-learning-dna, xiaozhi-math-error-gene-archive, xiaozhi-physics-error-archive
---
...
⚠️ **【学科边界声明】**：本SKILL作为通用核心，负责接收所有科目的错题并提供统一的四维错因分析。**但是，当检测到属于“数学”学科的错题时，本SKILL只负责基础记录，深度的错因分析（如概念混淆对照、读题训练、数学焦虑处理）将无缝交由专用的“数学错误基因档案”SKILL处理，以避免两边重复记录和触发冲突。同理，当检测到属于“物理”学科的错题时，本SKILL只负责基础记录和通用层四维分类，深度的五维子类型定位（P类图景建立/C类概念混淆/F类公式误用/R类过程分析/T类数学工具）、跨维度关联分析和顽固弱项突破将无缝交由专用的“物理错误基因档案”SKILL处理。**
...
触发一：同一知识点 + 同一错误类型连续出现3次
  ...
  ⚠️ 【数学例外】：数学科目的3次触发，本SKILL仅标记为待基因档案处理，不独立启动预警响应流程。由数学错误基因档案执行顽固弱项识别与专项突破。

  ⚠️ 【物理例外】：物理科目的3次触发，本SKILL仅标记为待物理基因档案处理，不独立启动预警响应流程。由物理错误基因档案执行物理版顽固弱项突破流程（纯净版图景测试→规则重建→验证锁定→写入档案）。
...
■ 错误类型全景（通用四维标签）
  ...
  审题习惯问题：[X]%
  策略选择错误：[X]%
  注：数学科目的深度分类（概念模糊/计算失误/读题失误/方法用错及子类型ID）由数学错误基因档案提供
  注：物理科目的深度分类（P类图景建立/C类概念混淆/F类公式误用/R类过程分析/T类数学工具及子类型ID）由物理错误基因档案提供

■ 三大顽固弱项
  弱项①：[知识点] —— 出现[N]次，状态：[已攻克 / 突破中 / 待处理]
  置信度评估：[🟢数据充分 / ⚠️初步判断 / 🔴样本量不足]
  📌 【数学弱项数据来源】：数学科目的顽固弱项、子类型ID、跨维度关联、突破状态，均由数学错误基因档案提供。本报告仅做汇总呈现，不独立分析数学弱项。
  📌 【物理弱项数据来源】：物理科目的顽固弱项、子类型ID、跨维度关联、突破状态，均由物理错误基因档案提供。本报告仅做汇总呈现，不独立分析物理弱项。
...
错题本
    ...
    <-> 数学错误基因档案（数学错题交接与深度分析回写）
    <-> 物理错误基因档案（物理错题交接与深度分析回写）
```

### 9.2 与数学错误基因档案的协作协议

本SKILL与"数学错误基因档案"构成**通用层→专属层**的纵向协作关系，协议如下：

#### 9.2.1 交接触发条件

当满足以下任一条件时，本SKILL向数学错误基因档案推送交接记录：
...
档案中"错误类型"字段值为 [概念/计算/审题/策略]，交接时需映射为 `mathBasicDimension` 的标准值：
...
```
注：historyRefs 仅在"顽固弱项确认"类型时填写。

#### 9.2.3 回写机制

数学错误基因档案完成深度分析后，向本SKILL回写以下字段：
...
本SKILL与"物理错误基因档案"构成**通用层→专属层**的纵向协作关系，协议如下：

#### 9.3.1 交接触发条件

当满足以下任一条件时，本SKILL向物理错误基因档案推送交接记录：
...
档案中"错误类型"字段值为 [概念/计算/审题/策略]，交接时需映射为 `physicsBasicDimension` 的标准值（通用四维→物理五维为非1:1映射）：

```text
档案标签 → 交接标签映射：
  概念 → 图景建立(P) 或 概念混淆(C)  [判断线索：是否缺失画图景习惯]
  计算 → 数学工具错误(T)
  审题 → 图景建立(P) 或 概念混淆(C)  [判断线索：审题遗漏是否与图景缺失有关]
  策略 → 公式误用(F) 或 过程分析错误(R)  [判断线索：公式选错归F，过程拆分不完整归R]
```
...
交接记录结构：
{
  "source": "wrong-answer-notebook",
  "type": "新错题 / 顽固弱项确认 / 深度分析请求 / 焦虑信号",
  "physicsBasicDimension": "图景建立(P) / 概念混淆(C) / 公式误用(F) / 过程分析错误(R) / 数学工具错误(T)",
...
```
注：historyRefs 仅在"顽固弱项确认"类型时填写。judgmentClue 在"新错题"类型时必填，帮助物理基因档案快速定位子类型。

#### 9.3.3 回写机制

物理错误基因档案完成深度分析后，向本SKILL回写以下字段：
...
```text
场景一：物理科目3次同类错误预警
  本SKILL：仅标记"待物理基因档案处理"，不启动§五预警响应流程
  物理基因档案：执行物理版顽固弱项突破流程（纯净版图景测试→规则重建→验证锁定→写入档案）
...
场景三：物理科目弱项专项提醒
  物理基因档案：指定提醒需求（何时提醒、提醒内容）
  本SKILL：统一调度IM提醒（避免多个SKILL同时提醒学生）

场景四：物理焦虑信号词检测
  本SKILL：检测到信号词后转交物理基因档案
  物理基因档案：执行"数据替代情绪+图景重建"四步流程（确认感受-调取数据-图景重建+具体任务-执行）
```
