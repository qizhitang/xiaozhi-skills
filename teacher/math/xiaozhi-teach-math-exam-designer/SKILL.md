...
description: >
  数学教师的测评设计（高中目前覆盖预备知识与函数）：用双向细目表把"凭感觉出数学卷"变成可诊断的命题。
...
依据《普通高中数学课程标准日常修订版（2017 年版 2025 年修订）》实施建议中的"学业水平考试与高考建议"，以及必修主题一"预备知识"、主题二"函数"。高一单元测评、期中与期末测评按下面的口径设计；几何与代数、概率与统计和选择性必修的测评，如实说明暂不覆盖。

- **范围**：学业水平考试的内容范围是必修课程，高考的内容范围是必修和选择性必修课程。高一衔接段的测评只考预备知识与函数。
- **难度依据**：学业质量水平一是学业水平考试命题的依据，高考命题不超过水平二。据此，本 SKILL 建议高一的形成性测评以水平一为主、少量试题到水平二（这是本 SKILL 的建议，不是课标的规定，老师可按班情调整）。
- **题型**：选择题（单项选择题与多项选择题）、填空题和解答题；学业水平考试的解答题分值占比不少于 40%；应有一定数量的实际应用问题，可以探索结构不良问题、开放性问题。课标要求逐步减少选择题、填空题的题量，适度增加试题的思维量，强调通性通法，淡化解题技巧。多项选择题在工作空间里 `itemType` 仍记"选择"（schema 没有单独的枚举），在细目表上注明"多选"。
- **细目表**：纵轴照用第五节的认知四级（高中课标的内容要求同样用"了解、理解、掌握"等行为动词），另加"核心素养""学业质量水平"两列给老师看。这两列只写在细目表上，不写进工作空间（schema 没有这两个字段）。
- **本地口径**：本省学业水平考试、高考的题型、分值与时长，以本地当年的考试说明为准，本 SKILL 不内置任何省份的试卷结构。
- 题目来源与版权、AI 生成题的自检与标注、试题保密照第六节与开头的"试题保密"段执行；启用前的统考试题不得输入。
